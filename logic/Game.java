...
public class Game {
   ...
   *
   * <p>This saves time when the player clicks a map to when they can start playing.
   */
  private void loadControlPanel() {
    controlPanelFuture =
        MyExecutor.submit(
            new Callable<ControlPanel>() {
              @Override
  ...
  private void updateLevelStats() {
    ...
    String timeBetweenCreeps =
        "0 - "
            + Helper.format(
                Formulae.ticksBetweenAddCreep(level) * 2 / Constants.CLOCK_TICKS_PER_SECOND, 2)
            + "s";
  ...
  private class Clock extends Thread {
    ...
    private final int[] fastModes = new int[] {1, 2, 10, 100};
    ...
    private Creep getNewCreep() {
      ...
      if (creepsToAdd > 0) {
        if (addCreepIn < 1) { // If the time has got to zero, add a creep
          creep = new Pacman(level, levelHP, Collections.unmodifiableList(gameMap.getPathPoints()));
  ...
  public class ControlEventProcessor {
    ...
    public void processEndLevelUpgradeButtonPress(
        boolean livesUpgrade, boolean interestUpgrade, boolean moneyUpgrade, Attribute a) {
    ...
    public void processEndLevelUpgradeButtonRollover(
        boolean livesUpgrade,
        boolean interestUpgrade,
        boolean moneyUpgrade,
        Attribute a,
        boolean on) {
      if (on) {
    ...
    public void processSellButtonRollover(boolean on) {
      Tower toAffect = towerToAffect();
      if (toAffect != null && on) {
        controlPanel.updateCurrentCost(
            "Sell " + toAffect.getName(), scene.getTowerSellValue(toAffect));
      } else {
        controlPanel.clearCurrentCost();
      }
    }
    ...
    private Tower towerToAffect() {
      return selectedTower != null ? selectedTower : rolloverTower != null ? rolloverTower : null;
    }
  }
}
