...
public class ImageHelper {
   ...
   *
   * <p>The path must be relative and it is loaded from the images folder.
   *
   * <p>For instance, to get images/other/bullet.png use {"other", "bullet.png"}.
   */
  public static BufferedImage loadImage(String... foldersAndFileName) {
    URL imageURL = createImageURL(foldersAndFileName);
    if (imageURL == null) {
      throw new IllegalArgumentException(
          "Image: '" + Arrays.asList(foldersAndFileName) + "' not found.");
    }
    try {
  ...
  /**
   * Makes a new BufferedImage that is a scaled version of that given with specified quality.
   *
  ...
  public static BufferedImage rotateImage(BufferedImage image, double angle) {
    BufferedImage temp =
        new BufferedImage(image.getWidth(), image.getHeight(), getImageType(image));
    AffineTransform at =
  ...
  public static void writePNG(BufferedImage image, String... foldersAndFileName) {
    ...
    try {
      ...
      f =
          new File(
              createImageURL(
                      Arrays.copyOfRange(foldersAndFileName, 0, foldersAndFileName.length - 1))
                  .toURI());
