...
public abstract class AbstractTower implements Tower {
  ...
  private static final Map<Class<? extends AbstractTower>, Map<LooseFloat, BufferedImage>>
      rotatedImages = new HashMap<Class<? extends AbstractTower>, Map<LooseFloat, BufferedImage>>();
  ...
  protected AbstractTower(
    ...
    overlayImage =
        hasOverlay
            ? loadImage(overlayImages, width, "towers", "overlays", className + "Overlay.png")
            : null;
  ...
  public ExperienceReport getExperienceReport() {
    ...
    return new ExperienceReport(
        level, kills, nextUpgradeKills, (long) damageDealt, (long) nextUpgradeDamage);
  }
  ...
  public void setCreepComparator(Comparator<Creep> c) {
    if (c instanceof DistanceComparator) {
      creepComparator = new DistanceComparator(centre, ((DistanceComparator) c).isClosestFirst());
    } else {
      creepComparator = c;
    }
  }
  ...
  protected List<Bullet> fireBulletsAt(
      Creep c,
      Point p,
      boolean rotateTurret,
      int turretWidth,
      double range,
      double bulletSpeed,
      double damage) {
  ...
  private BufferedImage drawOverlayOnBaseImage(double angle) {
    ...
    AffineTransform at =
        AffineTransform.getRotateInstance(angle, image.getWidth() / 2, image.getHeight() / 2);
  ...
  private static BufferedImage createRotatingImage(int width, int turretWidth) {
    ...
    BufferedImage turretCentre =
        ImageHelper.loadImage("towers", "overlays", "turrets", "turretCentre.png");
    BufferedImage image = new BufferedImage(width, width, BufferedImage.TYPE_INT_ARGB_PRE);
    Graphics2D g = image.createGraphics();
    g.drawImage(
        turretCentre, width / 2 - turretThickness / 2, width / 2 - turretThickness / 2, null);
    g.setColor(Color.BLACK);
    g.fillRect(
        width / 2 - turretThickness / 2, width / 2 - turretWidth, turretThickness, turretWidth);
  ...
  public static void main(String... args) {
    for (int i = 1; i <= 25; i++) {
      ImageHelper.writePNG(
          createRotatingImage(50, i),
          "towers",
          "rotatingOverlays",
          "turrets",
          "turret" + i + ".png");
